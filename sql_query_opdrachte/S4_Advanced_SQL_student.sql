...
FROM medewerkers 
WHERE gbdatum < '1980-12-31' AND (functie = 'TRAINER' or functie = 'VERKOPER');
...
SELECT u.cursus code, u.begindatum begindatum, count(*) aantal_inschrijvingen
FROM inschrijvingen i
INNER JOIN uitvoeringen as u on i.cursus = u.cursus and i.begindatum = u.begindatum
WHERE i.begindatum between '2019-01-01' and '2019-12-31' group by u.cursus, u.begindatum
having count(*)>=3;
...
SELECT cursist medewerkernummer, cursus cursuscode from inschrijvingen group by cursist, cursus having count(cursus)>1;
...
SELECT MAX(EXTRACT(YEAR FROM age(gbdatum))) - MIN(EXTRACT(YEAR FROM age(gbdatum))) AS verschil,
...
SELECT
    ...
    ROUND(AVG(COALESCE(comm, 0))) AS commissie_medewerkers,
    ROUND(AVG(CASE WHEN functie = 'VERKOPER' THEN COALESCE(comm, 0) ELSE null END)) AS commissie_verkopers
FROM medewerkers;                                              
